...
susie.locus.res <- run_finemapping(sumstats.locus, LD_matrices = LD_matrices, priortype = 'uniform', n = n, L = 1)

# add susie PIP information to GWAS summary stats
susie.sumstats.locus <- merge_susie_sumstats(susie.locus.res, sumstats.locus)

head(susie.sumstats.locus)
...
```{r diagnosis-plots, fig.width=10, fig.height=8}
...
cat(sprintf("%d out of %d flipped variants detected. \n", length(intersect(detected_index, flip_index)), length(detected_index)))

condz$conditional_dist[union(flip_index, detected_index),]

p1 <- ggplot(condz$conditional_dist, aes(x = condmean, y = z, col = factor(flipped))) +
  geom_point() +
  scale_colour_manual(values = c("0" = "black", "1" = "red")) + 
  labs(x = "Expected", y = "Observed z scores", color = "Allele flipped") + 
  theme_bw()

p2 <- ggplot(condz$conditional_dist, aes(x = condmean, y = z, col = factor(detected))) +
  geom_point() +
  scale_colour_manual(values = c("0" = "black", "1" = "red")) + 
...
susie.locus.res <- run_finemapping(sumstats.locus.flip, LD_matrices = LD_matrices, priortype = 'uniform', n = n, L = 1)
